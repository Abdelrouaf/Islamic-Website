...
export default function CreateTopicInMonotheism() {
...
const saveData = async () => {
    ...
    if (!title || !description || !image || (image && !(image instanceof File))) {
        showToast('Invalid input, title & description and image must be not empty', 'invalid');
        return;
    }
    ...
    try {
        ...
        const response = await axios.post('http://147.79.101.225:2859/api/monotheismBlog/', 
            form, 
                {
                    headers: {
