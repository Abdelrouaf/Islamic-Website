...
export default function HaijTopic() {
    ...
    useEffect(() => {
        const fetchTopic = async () => {
            try {
                const response = await fetch(`http://localhost:8080/api/haijBlog/${id}`);
                if (!response.ok) {
    ...
    const handleUpdate = async () => {
        try {
            ...
            const response = await axios.put(`http://localhost:8080/api/haijBlog/${id}`, payload);
            ...
            if (response.status === 200) {
                ...
                navigate(`/en/pillars/haij/topic/${id}`);
        ...
        if (confirmDelete) {
            try {
                ...
                if (response.ok) {
                    ...
                    navigate('/en/pillars/haij/create'); // Navigate back after deletion
                    window.location.reload();
                } else {
