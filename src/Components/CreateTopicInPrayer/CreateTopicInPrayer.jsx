...
export default function CreateTopicInPrayer() {
    const navigate = useNavigate();

    // State for the current form data
    const [formData, setFormData] = useState({
        id: uuid(), // Generate new ID for new topic
        image: '',
        imageURL:'',
        title: '',
        description: '',
...
const checkTitleExists = async (title) => {
    try {
        ...
        const existingTopics = data.PrayerBlog || [];
...
const saveData = async () => {
    ...
    try {
        ...
        const response = await axios.post('http://147.79.101.225:2859/api/prayerBlog/', 
            form, 
                {
                    headers: {
