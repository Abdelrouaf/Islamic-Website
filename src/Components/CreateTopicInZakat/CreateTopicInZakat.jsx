...
export default function CreateTopicInZakat() {
  ...
  const checkTitleExists = async (title) => {
    try {
      ...
      const existingTopics = data.ZakatBlog || [];

      // Check if any topic has the same title (case-insensitive)
      return existingTopics.some(
        (topic) =>
          topic.title.trim().toLowerCase() === title.trim().toLowerCase()
      );
    } catch {
  ...
  const saveData = async () => {
    ...
    try {
      ...
      if (response.status === 200 || response.status === 201) {
        ...
        showToast(
          `Failed to add the topic: Unexpected error occurred while adding the topic.`,
          "error"
        );
      }
    } catch {
          ...
          >
            <div className={layout.inputs}>
              ...
              <div className="mb-4">
                <div className={layout.inputTitle}>
                  <h4>
                    <span style={{ color: "red" }}>* </span>topic title
                  </h4>
                </div>
              ...
              <div className="mb-4">
                <div className={layout.inputTitle}>
                  <h4>
                    <span style={{ color: "red" }}>* </span>topic description
                  </h4>
                </div>
            ...
            {/* Buttons */}
            <div
              className={`d-flex justify-content-end align-items-center gap-3 ${layout.btns}`}
            >
              <button
                type="reset"
                className={`btn btn-outline-success ${layout.backBtn}`}
              >
                Reset
              </button>
              <button
                type="submit"
                className={`btn btn-success ${layout.saveBtn}`}
